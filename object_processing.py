...
cameraMatrix = np.float64([[1, 0, int(np.pi / w_rad/deltaT_counter/2)],
                           [0, 1, int(np.pi / w_rad/deltaT_counter)/2],
                           [0, 0, 1]
                          ], dtype = np.float64)
...
class TrackedObject(object):
    ...
    def set_transformation(self, rotation_vector=None, transform_vector=None):
        ...
        self.transformed_UVW_points = np.dot(self.UVW_points, self.rotation_matrix)+transform_vector
        #
    ...
    def solvePnP(self):
        a = cv2.solvePnP(self.UVW_points, self.visible_point_coordinates.astype(np.float32), cameraMatrix,
...
if __name__=='__main__':
    ...
    cube.set_transformation([0, 0, 0], initial_object_pos)
    [_,r,t] = cube.solvePnP()

    cube.set_transformation(r.flatten()*180/np.pi,t.flatten())
