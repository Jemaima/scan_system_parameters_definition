...
results = results.append(
    pd.read_csv('experiment_logs/results_2018-04-30_11-23-21.csv', decimal=',', sep=';', index_col=0),
    ignore_index=True)
...
plt.colorbar()
plt.clim(vmin=0.0)
plt.yticks(np.arange(len(area_rot_error_dependence.index.values)), area_rot_error_dependence.index.values/1000)
plt.xticks(np.arange(len(area_rot_error_dependence.columns.values)), area_rot_error_dependence.columns.values / 1000)
plt.suptitle('translation error')

plt.figure(figsize=(7, 7))
area_rot_error_dependence = \
    results[results['noise'] == 0.0][results['translation_y'] == 0.0][results['error_r'] < 45].groupby(
        ['translation_x', 'translation_z'])[
        'error_r'].aggregate(np.max).unstack()
plt.imshow(area_rot_error_dependence, interpolation='nearest')
plt.ylabel('x,m', fontsize=12)
plt.xlabel('distance,m', fontsize=12)
plt.colorbar()
...
for n in noise_values[0:4]:
    plt.subplot(2, 2, 5 - i)
    area_translation_error_dependence = \
        results[results['noise'] == n][results['translation_y'] == 0.0][results['translation_z'] < 6000].groupby(
            ['translation_x', 'translation_z'])[
            'error_t'].aggregate(np.mean).unstack()
    plt.imshow(area_translation_error_dependence, interpolation='nearest')
    plt.ylabel('x,m', fontsize=12)
    plt.xlabel('distance,m', fontsize=12)
    plt.colorbar()
    plt.clim(vmin=0.0)
    plt.yticks(np.arange(len(area_translation_error_dependence.index.values)),
               area_translation_error_dependence.index.values/1000)
    plt.xticks(np.arange(len(area_translation_error_dependence.columns.values)),
               area_translation_error_dependence.columns.values / 1000)
    i = i + 1
...
for n in noise_values[0:4]:
    plt.subplot(2, 2, 5 - i)
    area_rot_error_dependence = \
        results[results['noise'] == n][results['translation_y'] == 0.0][results['translation_z'] < 6000][
            results['error_r'] < 45].groupby(['translation_x', 'translation_z'])[
    ...
    plt.colorbar()
    plt.clim(vmin=0.0)
    plt.yticks(np.arange(len(area_rot_error_dependence.index.values)),
               area_rot_error_dependence.index.values/1000)
    plt.xticks(np.arange(len(area_rot_error_dependence.columns.values)),
               area_rot_error_dependence.columns.values / 1000)
    i = i + 1
    plt.title(' '.join(['noise =', "{:4.1f}".format(n * op.deltaT_counter * 1000000), 'mcs']), fontsize=14)
plt.tight_layout()
...
# NOISE DEPENDENCE
# ==================================================
noise_dependence = results[results['translation_z'] < 6000][results['error_r'] < 45].groupby(
    ['noise'])['error_r', 'error_t'].aggregate(np.mean)
plt.figure()
plt.subplot(2, 1, 1)
...
plt.subplot(2, 1, 2)
plt.plot(np.tan(noise_dependence.index * op.deltaT_counter) * 1000000, noise_dependence['error_t'].values * 2)
plt.plot(np.tan(noise_dependence.index * op.deltaT_counter) * 1000000, [10] * len(noise_dependence['error_t'].values))
